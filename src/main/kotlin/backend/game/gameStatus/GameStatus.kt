 ...
 * @property [startingPopulation] the non-ordered subset of players which at the current turn never got the good.
 ...
 */
class GameStatus(
    val startingPopulation: Set<Player>,
) {

    val activePopulation: MutableSet<Player> = startingPopulation.filter { it !is RationalPlayer }.toMutableSet()
    val chain: MutableList<Player> = mutableListOf()
    var turn: Int = 0
    val numOfPlayers: Int = startingPopulation.size
